...
)
{
    ...
    case FS_FAT12 :
        bc = (WORD)clust * 3 / 2;
        if (!move_window(fs, fatsect + (bc / S_SIZ))) return iFALSE;
        p = &fs->win[bc & (S_SIZ - 1)];
    ...
    case FS_FAT16 :
        if (!move_window(fs, fatsect + (clust / (S_SIZ / 2)))) return iFALSE;
        ST_WORD(&fs->win[((WORD)clust * 2) & (S_SIZ - 1)], (WORD)val);
        break;

    case FS_FAT32 :
        if (!move_window(fs, fatsect + (clust / (S_SIZ / 4)))) return iFALSE;
...
)
{
    ...
    while (clust >= 2 && clust < fs->max_clust) {
        ...
        if (nxt == 1) return iFALSE;
        if (!put_cluster(fs, clust, 0)) return iFALSE;
...
)
{
    ...
    if ((idx & ((S_SIZ - 1) / 32)) == 0) {        /* Table sector changed? */
        ...
        if (!dirobj->clust) {        /* In static table */
            if (idx >= fs->n_rootdir) return iFALSE;    /* Reached to end of table */
